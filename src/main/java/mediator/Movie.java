...
public class Movie {
	...
	public void displayAllFilmInfoByNameActor(String actor) throws SQLException, Exception {
		...
		for (int i = 0; i < movies.size(); i++) {
			...
			if (containsSpecialChar(movies.get(i)) == false) {
				...
				// movie.getMovieByTitle(movies.get(i));
				// System.out.println(movie.display());
				ArrayList<Movie> tabs = movie.getMovieInfo(movies.get(i));
	...
	public ArrayList<Movie> getMovieInfo(String name) throws Exception{
	...
	public void printAllMoviesByActorName(String actor ) throws Exception {
		...
		for (int i = 0; i < movies.size();i++) {
			if (containsSpecialChar(movies.get(i)) == false) {
			ArrayList<Movie> moviesInfo = getMovieInfo(movies.get(i));
			for (int j = 0; j < moviesInfo.size(); j++) {				
				...
				ArrayList<Movie> moviesInfo = getMovieInfo(refactorSpecialChar(movies.get(i)));
				for (int j = 0; j < moviesInfo.size(); j++) {				
					System.out.println(moviesInfo.get(j));
				}
	...
	//Movie movie = new Movie();	
	//String x = "it's can be' hak ";
	//System.out.println(movie.refactorSpecialChar(x));
